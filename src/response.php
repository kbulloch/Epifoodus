<?php
	...
	class Response
	{
		...
		function save()
		{
			$statement = $GLOBALS['DB']->query("INSERT INTO likes (answer, restaurant_id, user_id) VALUES ({$this->getAnswer()}, '{$this->getRestaurantId()}', {$this->getUserId()}) RETURNING id;");
	    ...
	    {
	        $all_responses = $GLOBALS['DB']->query("SELECT * FROM likes;");
	    ...
	    {
	    	 $GLOBALS['DB']->exec("DELETE FROM likes *;");
	    }

	    function update($new_answer)
	    {
	        $GLOBALS['DB']->exec("UPDATE likes SET answer = '{$new_answer}' WHERE id = {$this->getId()};");
	        $this->setAnswer($new_answer);
	    }

	    function delete()
	    {
	        $GLOBALS['DB']->exec("DELETE FROM likes WHERE id = ({$this->getId()});");
	    }
	}
