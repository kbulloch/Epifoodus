...
restaurants.id) WHERE likes.answer = 2 AND likes.user_id = {$this->getId()};");
	...
	//
    // function save()
	// {  $statemnt= $GLOBALS['DB']->query("INSERT INTO users (username, password) VALUES
...
//    	function getLikes()
// 	{
//        $user_likes= $GLOBALS['DB']->exec("SELECT restaurants.* FROM
// 			users JOIN likes ON  (users.id = likes.user_id)
